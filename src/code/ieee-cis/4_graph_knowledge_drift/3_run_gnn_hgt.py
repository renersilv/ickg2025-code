"""
Script para treinar e avaliar modelo HGT nos grafos de conhecimento construídos 
...
- Modelo: HGT (Heterogeneous Graph Transformer)
...
- Definição de arquitetura HGT especializada
...
Data: 2025-06-29 (Adaptado para HGT específico)
Projeto: GraphSentinel 2.0 - Detecção de Fraude com HGT
Versão: 6.0 (HGT Framework - IEEE-CIS Dataset Heterogêneo)
"""
...
class HGTModel(nn.Module):
    """
    Heterogeneous Graph Transformer (HGT) para grafos heterogêneos.
    ...
    def __init__(self, data: HeteroData, hidden_channels: int = 64, out_channels: int = 1, 
                 num_layers: int = 2, dropout: float = 0.3, use_gradient_checkpointing: bool = True):
        ...
        logging.info(f"🔧 HGT KNOWLEDGE: node_types={len(self.node_types)}, edge_types={len(self.edge_types)}")
        logging.info(f"   Node types: {list(self.node_types)}")
        logging.info(f"   Edge types: {list(self.edge_types)}")
        
        # >>> ADIÇÃO: Módulo de Normalização para estabilizar inputs do HGT <<<
        self.input_norms = nn.ModuleDict()
        for node_type in data.node_types:
            self.input_norms[node_type] = nn.LayerNorm(hidden_channels)
        # >>> FIM DAS LINHAS NOVAS <<<
        
        # Camadas HGT operando nativamente em estrutura heterogênea
        ...
        for _ in range(num_layers):
            conv = HGTConv(
                ...
                hidden_channels,
                data.metadata(),
                heads=8  # Reduzido agressivamente para 2 heads para máxima economia de memória
            )
        ...
        # O HGT gera embeddings ricos para todos os tipos de nós
    ...
    def forward(self, data: HeteroData) -> torch.Tensor:
        """
        Forward pass para HGT heterogêneo.
        ...
        # >>> ADIÇÃO: Normaliza e Ativa os Inputs para o HGT <<<
        ...
        # PASSO 2: Passagem pelas camadas HGT mantendo estrutura heterogênea COM CONEXÕES RESIDUAIS
        for i, conv in enumerate(self.convs):
            ...
            for node_type in x_dict:
                ...
                if torch.isnan(x_dict[node_type]).any() or torch.isinf(x_dict[node_type]).any():
                    logging.warning(f"⚠️ NaN/Inf detectado na camada HGT {i} para {node_type}. Aplicando correção.")
...
def ensure_model_reproducibility(model: nn.Module, model_name: str, model_seed: int) -> None:
    """
    Garante reprodutibilidade científica com inicializações padrão para HGT.
    
    Args:
        model (nn.Module): Modelo HGT a ser inicializado
    ...
    # 🔧 INICIALIZAÇÃO TÉCNICA: HGT não precisa de forward pass especial
        ...
        for name, param in model.named_parameters():
            ...
            if hasattr(param, 'dim'):
                ...
                if 'weight' in name and param_dim >= 2:
                    ...
                    if 'HGT' in model_name:
                        # HGT: Xavier/Glorot uniform (recomendado para transformers)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Treinamento de modelo HGT para o Knowledge Graph IEEE-CIS",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplo de uso - IEEE-CIS HGT Knowledge Graph:
  # Treinar HGT:
  python 3_run_gnn_hgt.py --graph-type knowledge_hetero
  
  # Treinar com configurações específicas:
  python 3_run_gnn_hgt.py --epochs 200 --patience 30
...
Modelo:
  - HGT: Heterogeneous Graph Transformer (multi-relacional com atenção)
  ...
  - knowledge_hetero_data.pt (HeteroData com múltiplos tipos de nós/arestas)
    ...
    * Suporte completo a grafos heterogêneos com PyTorch Geometric
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help='Número de canais ocultos para HGT (padrão: 64, reduzido para eficiência de memória)'
    )
...
def main():
    ...
    log_file = f"logs/{graph_type}_hgt_training_seed{seed_suffix}.log"
    ...
    logging.info("="*80)
    logging.info(f"INICIANDO TREINAMENTO DE MODELO HGT - {graph_type.upper()}")
    logging.info("METODOLOGIA MULTISTATGRAPH FRAMEWORK")
    logging.info("="*80)
    logging.info(f"🎯 Tipo de grafo: {graph_type}")
    logging.info(f"🤖 Modelo: HGT")
    ...
    # Configuração do modelo HGT para grafos de conhecimento
    model_config = {
        'name': 'HGT',
        'class': HGTModel,
        'params': {
    ...
    results_file = f"results/{graph_type}_hgt_performance_seed{seed_suffix}.json"
    ...
    if all_results and hasattr(hetero_data['transaction'], 'monitoring_mask'):
        ...
        if drift_results:
            drift_file = f"results/{graph_type}_hgt_drift_analysis_seed{seed_suffix}.json"
    ...
    if all_results:
        ...
        logging.info(f"\n🏆 MODELO HGT: {model_result[0]} (F1-Score: {model_result[1]['test_metrics']['f1_score']:.4f})")
    
    logging.info("="*80)
    logging.info(f"✅ TREINAMENTO HGT {graph_type.upper()} CONCLUÍDO COM SUCESSO!")
