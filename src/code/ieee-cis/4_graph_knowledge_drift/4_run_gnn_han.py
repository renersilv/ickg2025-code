"""
Script para treinar e avaliar modelo HAN nos grafos de conhecimento construídos 
...
- Modelo: HAN (Heterogeneous Attention Network)
...
- Definição de arquitetura HAN especializada
...
Data: 2025-06-29 (Adaptado para HAN específico)
Projeto: GraphSentinel 2.0 - Detecção de Fraude com HAN
Versão: 6.0 (HAN Framework - IEEE-CIS Dataset Heterogêneo)
"""
...
class HANModel(nn.Module):
    """
    Heterogeneous Attention Network (HAN) para grafos heterogêneos.
    
    METODOLOGIA KNOWLEDGE GRAPH HETEROGÊNEO:
    - Processa diferentes tipos de relacionamentos (7 tipos de arestas)
    - Mecanismo de atenção multi-head especializado por tipo de relação
    ...
    def __init__(self, data: HeteroData, hidden_channels: int = 64, out_channels: int = 1, 
                 num_layers: int = 2, num_heads: int = 4, dropout: float = 0.3, use_gradient_checkpointing: bool = True):
        ...
        # Projeções lineares para cada tipo de nó
        self.node_lin = nn.ModuleDict()
        for node_type in data.node_types:
            if hasattr(data[node_type], 'x') and data[node_type].x is not None:
                node_in_channels = data[node_type].x.shape[1]
                self.node_lin[node_type] = Linear(node_in_channels, hidden_channels)
            else:
                # Para nós sem features, cria embeddings aprendíveis
                self.node_lin[node_type] = nn.Embedding(data[node_type].num_nodes, hidden_channels)
        
        logging.info(f"🔧 HAN KNOWLEDGE: node_types={len(self.node_types)}, edge_types={len(self.edge_types)}")
        logging.info(f"   Node types: {list(self.node_types)}")
        logging.info(f"   Edge types: {list(self.edge_types)}")
        
        # >>> ADIÇÃO: Módulo de Normalização para estabilizar inputs do HAN <<<
        self.input_norms = nn.ModuleDict()
        for node_type in data.node_types:
            self.input_norms[node_type] = nn.LayerNorm(hidden_channels)
        # >>> FIM DAS LINHAS NOVAS <<<
        
        # Camadas HAN operando nativamente em estrutura heterogênea
        # OTIMIZAÇÃO DE MEMÓRIA: Reduz número de heads para economizar memória GPU
        ...
        # >>> MELHORIA: Classificador com embeddings concatenados (transaction + card) <<<
        # O HAN gera embeddings ricos para todos os tipos de nós
        # Vamos usar embeddings de transação + card para decisão mais informada
        classifier_in_channels = hidden_channels * 2  # Transação + Cartão
        
        self.classifier = nn.Sequential(
            nn.Linear(classifier_in_channels, hidden_channels),  # Ajustado para entrada concatenada
        ...
        # >>> ADIÇÃO: Módulo de agregação para embeddings de cartão específicos por transação <<<
        self.card_aggr = MeanAggregation()
        # >>> FIM DA LINHA NOVA <<<
    ...
    def forward(self, data: HeteroData) -> torch.Tensor:
        """
        Forward pass para HAN heterogêneo.
        ...
        for node_type in self.node_types:
            if hasattr(data[node_type], 'x') and data[node_type].x is not None:
                ...
                x_dict[node_type] = self.node_lin[node_type](features)
            else:
                # Nós sem features: usa embeddings aprendíveis
                num_nodes = data[node_type].num_nodes
                node_indices = torch.arange(num_nodes, device=device)
                x_dict[node_type] = self.node_lin[node_type](node_indices)
        
        # >>> ADIÇÃO: Normaliza e Ativa os Inputs para o HAN <<<
        for node_type, x in x_dict.items():
            # Aplica LayerNorm para estabilizar a escala entre os diferentes tipos de nós
            x_normalized = self.input_norms[node_type](x)
            # Aplica a função de ativação
            x_dict[node_type] = x_normalized.relu()
        # >>> FIM DAS LINHAS NOVAS <<<
        
        # PASSO 2: Passagem pelas camadas HAN mantendo estrutura heterogênea COM CONEXÕES RESIDUAIS
        for i, conv in enumerate(self.convs):
            # >>> ADIÇÃO: Salva o estado atual antes da convolução para conexão residual <<<
            x_dict_prev = {node_type: x.clone() for node_type, x in x_dict.items()}
            
            # Verifica se deve usar gradient checkpointing
            use_checkpoint = (
                self.use_gradient_checkpointing and 
                self.training and 
                any(p.requires_grad for p in conv.parameters()) and
                any(x.requires_grad for x in x_dict.values() if isinstance(x, torch.Tensor))
            )
            
            if use_checkpoint:
                # Usa gradient checkpointing para economizar memória durante treinamento
                def checkpoint_conv(x_dict, edge_index_dict):
                    return conv(x_dict, edge_index_dict)
                x_dict = checkpoint(checkpoint_conv, x_dict, data.edge_index_dict, use_reentrant=False)
            else:
                x_dict = conv(x_dict, data.edge_index_dict)
            
            # >>> ADIÇÃO: Aplica conexão residual SOMA o output com o input da camada <<<
            for node_type in x_dict:
                x_dict[node_type] = x_dict_prev[node_type] + x_dict[node_type]  # CONEXÃO RESIDUAL
                ...
                if torch.isnan(x_dict[node_type]).any() or torch.isinf(x_dict[node_type]).any():
                    logging.warning(f"⚠️ NaN/Inf detectado na camada HAN {i} para {node_type}. Aplicando correção.")
                    x_dict[node_type] = torch.nan_to_num(x_dict[node_type], nan=0.0, posinf=1.0, neginf=-1.0)
        
        # PASSO 3: Concatena embeddings de transação e card para classificação mais rica
        transaction_embeddings = x_dict['transaction']
        
        # >>> MELHORIA OTIMIZADA: Usa agregação específica para embeddings de card por transação <<<
        # Em vez de média global, agrega embeddings de cartão específicos para cada transação
        if 'card' in x_dict and ('transaction', 'uses_card', 'card') in data.edge_index_dict:
            card_embeddings = x_dict['card']
            edge_index = data.edge_index_dict[('transaction', 'uses_card', 'card')]
            
            if edge_index.size(1) > 0 and card_embeddings.size(0) > 0:
                source_nodes, target_nodes = edge_index[0], edge_index[1]
                
                # Agrega os embeddings dos cartões (target_nodes) para cada transação (source_nodes)
                aggregated_card_emb = self.card_aggr(
                    card_embeddings[target_nodes], 
                    index=source_nodes, 
                    dim_size=transaction_embeddings.size(0)
                )
            else:
                # Fallback: zeros se não há arestas ou embeddings de card
                aggregated_card_emb = torch.zeros_like(transaction_embeddings)
        else:
            # Fallback: duplica transaction embeddings para manter dimensionalidade
            aggregated_card_emb = transaction_embeddings
        
        # Concatena embeddings de transação com os embeddings agregados dos seus cartões
        combined_embeddings = torch.cat([transaction_embeddings, aggregated_card_emb], dim=-1)
        
        # PASSO 4: Classificação final com embeddings combinados
...
def ensure_model_reproducibility(model: nn.Module, model_name: str, model_seed: int) -> None:
    """
    Garante reprodutibilidade científica com inicializações padrão para HAN.
    
    Args:
        model (nn.Module): Modelo HAN a ser inicializado
    ...
    # 🔧 INICIALIZAÇÃO TÉCNICA: HAN não precisa de forward pass especial
            ...
            if hasattr(param, 'dim'):
                ...
                if 'weight' in name and param_dim >= 2:
                    ...
                    if 'HAN' in model_name:
                        # HAN: Xavier/Glorot uniform (recomendado para transformers)
...
def train_one_epoch(model: nn.Module, data: HeteroData, optimizer: torch.optim.Optimizer, 
    ...
    if scaler is not None:
        with torch.amp.autocast('cuda'):
            ...
            # >>> MUDANÇA: Calibração manual para focar em Precision <<<
            # Em vez de usar pos_weight calculado automaticamente, usa valor fixo menos agressivo
            ...
            if len(train_class_counts) >= 2 and train_class_counts[1] > 0:
                # Calcula o peso original para referência (mas não usa)
                pos_weight_calculated = torch.clamp(train_class_counts[0].float() / train_class_counts[1].float(), min=1.0, max=100.0)
            else:
                pos_weight_calculated = torch.tensor(25.0, device=out_masked.device)
            
            # >>> OTIMIZAÇÃO: pos_weight refinado para melhor balance Precision/Recall <<<
            # Valor reduzido de 5.0 para 3.0 para ser mais conservador e aumentar Precision
            pos_weight = torch.tensor(3.0, device=out_masked.device)  # Otimizado para melhor F1-Score
            
            # Clipping do output do modelo para estabilidade numérica
            out_clamped = torch.clamp(out_masked, min=-15.0, max=15.0)
            
            # 🚀 LOSS FUNCTION CALIBRADA PARA MELHOR PRECISION
            # Cálculo da perda com pos_weight calibrado para balance Precision/Recall
            loss = F.binary_cross_entropy_with_logits(
                out_clamped, 
                targets_masked, 
                pos_weight=pos_weight  # Usando peso calibrado (5.0) em vez do calculado
            )
            
            # Calcula métricas rápidas para monitoramento das épocas
            with torch.no_grad():
                ...
                if len(np.unique(y_true)) > 1 and not np.isnan(y_probs).any():
                    try:
                        ...
                        logging.info(f"📊 Época {epoch}: BCE estável implementado")
                else:
                    logging.info(f"📊 Época {epoch}: BCE estável implementado")
            
            # Adicionar regularização L2 (opcional, mas recomendado)
            l2_reg = 0
            for param in model.parameters():
                if param.requires_grad:
                    l2_reg += torch.norm(param, 2)
            
            # Perda total com regularização L2
            reg_strength = 1e-5
            total_loss = loss + reg_strength * l2_reg
        ...
        # >>> OTIMIZAÇÃO: pos_weight refinado para melhor balance Precision/Recall <<<
        # Valor aumentado de 3.0 para 5.0 para elevar threshold ótimo e melhorar calibração
        pos_weight = torch.tensor(5.0, device=out_masked.device)  # Otimizado para elevar threshold e melhorar calibração
...
def parse_arguments():
    """
    Parse command line arguments for HAN training specification.
    
    Returns:
        argparse.Namespace: Parsed arguments containing graph_type and other parameters
    """
    parser = argparse.ArgumentParser(
        description="Treinamento de modelo HAN para o Knowledge Graph IEEE-CIS",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplo de uso - IEEE-CIS HAN Knowledge Graph:
  # Treinar HAN:
  python 4_run_gnn_han.py --graph-type knowledge_hetero
  
  # Treinar com configurações específicas:
  python 4_run_gnn_han.py --epochs 200 --patience 30 --num-heads 8
...
Modelo:
  - HAN: Heterogeneous Attention Network (multi-relacional com atenção)
  
Arquivo processado automaticamente:
  Knowledge Graph Heterogêneo (com arestas reversas para HAN):
  - knowledge_hetero_with_reverse_edges.pt (HeteroData com múltiplos tipos de nós/arestas + reversas)
    * Estrutura heterogênea: Multi-relacional, multi-entidade com arestas bidirecionais
    ...
    * Arestas reversas: Otimizado para HAN com tipos separados (uses_card + rev_uses_card)
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help='Número de canais ocultos para HAN (padrão: 64, reduzido para eficiência de memória)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help='Número de cabeças de atenção para HAN (padrão: 4)'
    )
    
    parser.add_argument(
        '--no-gradient-checkpointing',
        action='store_true',
        help='Desabilita gradient checkpointing (economiza CPU mas usa mais memória)'
    )
...
def main():
    """
    Função principal para treinamento e avaliação do modelo HAN.
    """
    ...
    log_file = f"logs/{graph_type}_han_training_seed{seed_suffix}.log"
    ...
    config = {
        ...
        'num_layers': 3,
        'num_heads': num_heads,  # Heads para HAN (valor do argumento)
        'dropout': dropout,
    ...
    # Carrega o grafo heterogêneo COM ARESTAS REVERSAS para HAN
    base_dir = "data/graph/ieee-cis"
    global_graph_path = os.path.join(base_dir, "knowledge_hetero_with_reverse_edges.pt")
    ...
    # Configuração do modelo HAN para grafos de conhecimento
    model_config = {
        'name': 'HAN',
        'class': HANModel,
        'params': {
    ...
    # � INICIALIZAÇÃO DE PARÂMETROS: Forward pass dummy para inicializar LazyModules
    logging.info("Inicializando parâmetros do modelo com forward pass dummy...")
    model.eval()
    with torch.no_grad():
        try:
            _ = model(hetero_data)
            logging.info("✅ Parâmetros inicializados com sucesso")
        except Exception as e:
            logging.warning(f"⚠️ Erro na inicialização dummy: {str(e)}")
    ...
    results_file = f"results/{graph_type}_han_performance_seed{seed_suffix}.json"
    ...
    if all_results and hasattr(hetero_data['transaction'], 'monitoring_mask'):
        ...
        if drift_results:
            drift_file = f"results/{graph_type}_han_drift_analysis_seed{seed_suffix}.json"
    ...
    if all_results:
        ...
        logging.info(f"\n🏆 MODELO HAN: {model_result[0]} (F1-Score: {model_result[1]['test_metrics']['f1_score']:.4f})")
    
    logging.info("="*80)
    logging.info(f"✅ TREINAMENTO HAN {graph_type.upper()} CONCLUÍDO COM SUCESSO!")
